...
public class DataLayerJDBC {
    ...
    public List<Integer> geefBestellingenGeleverdBinnen7Dagen() {
        ...
        try {
            ...
            ResultSet rs = stmt.executeQuery(Configuratie.queryLeveringInMinderDan7Dagen);
            while (rs.next()) {
    ...
    public List<Calendar> geefDatumsGeleverdInGent() {
        ...
        try {
            stmt = con.createStatement();
            ResultSet rs = stmt.executeQuery(Configuratie.queryDataLeveringBijKlantenUitGent);
            while (rs.next()) {
    ...
    public List<Artikel> geefArtikelenMinsten4xBesteld() {
        ...
        try {
            stmt = con.createStatement();
            ResultSet rs = stmt.executeQuery(Configuratie.queryArtikelenMin4BesteldOverMeerdereLeveringen);
            while (rs.next()) {
    ...
    public LinkedHashMap<Integer, Double> geefDuursteArtikelPerBestelling() {
        ...
        try {
            stmt = con.createStatement();
            ResultSet rs = stmt.executeQuery(Configuratie.queryDuursteArtikelPerBestelling);
            while (rs.next()) {
    ...
    public double geefTotaleFactuurprijs() {
        ...
        try {
            stmt = con.createStatement();
            ResultSet rs = stmt.executeQuery(Configuratie.queryTotalePrijsBestelling1);
            if (rs.next()) {
                som = rs.getDouble(1);
            }
        } catch (SQLException ex) {
